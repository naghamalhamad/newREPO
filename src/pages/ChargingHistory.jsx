        ...
        {chargingHistory.length === 0 ? (
          ...
          <ul className="flex flex-col gap-3">
            {chargingHistory.map((r) => (
              <li key={r.id}>
                <Link to={`/charge/history/${r.id}`} className="flex items-center justify-between rounded-card border border-line bg-surface p-4 active:bg-charge-tint/30">
                  <span>
                  ...
                  <span className="font-mono font-semibold text-charge">${r.cost.toFixed(2)}</span>
                </Link>
              </li>
            ))}
          </ul>
