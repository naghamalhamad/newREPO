  ...
  if (confirmed) {
    return (
      <div className="flex min-h-dvh flex-col items-center justify-center bg-stone px-6 text-center">
        ...
        <Link to="/home" className="mt-6 rounded-xl bg-brand px-6 py-3 font-heading font-semibold text-white">
          Back to Home
        </Link>
      </div>
    )
    ...
    <div className="flex min-h-dvh flex-col bg-stone pb-10">
      ...
      <main className="mx-auto flex w-full max-w-md flex-1 flex-col px-4 pt-3">
            ...
            {days.map((d) => (
              <button
                ...
                className={`flex flex-col items-center gap-0.5 rounded-xl border px-1 py-3 ${
                  day.key === d.key ? 'border-brand bg-brand text-white' : 'border-line bg-surface text-graphite'
                }`}
              >
                <span className="text-xs font-semibold">{d.label}</span>
                <span className={`text-[11px] ${day.key === d.key ? 'text-white/80' : 'text-mist'}`}>{d.sub}</span>
              </button>
            ))}
          </div>
        </section>
            ...
            {timeSlots.map((t) => (
              <button
                ...
                className={`rounded-xl border px-2 py-3 font-mono text-sm ${
                  time === t ? 'border-brand bg-brand text-white' : 'border-line bg-surface text-graphite'
                }`}
              >
                {t}
              </button>
        ...
        <button
          ...
          className="mt-auto w-full rounded-xl bg-brand py-4 text-center font-heading font-semibold text-white active:opacity-90 disabled:bg-line disabled:text-disabled-text"
        >
